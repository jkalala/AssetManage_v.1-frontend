...
async function getAssets(session: any): Promise<Asset[]> {
  try {
    if (!session?.user?.id) {
      console.log('No valid session in getAssets:', session)
      throw new Error('Unauthorized - No valid session')
    }

    // Get the base URL from the environment or use the current origin
    const baseUrl = process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3003'
    const apiUrl = `${baseUrl}/api/assets`
    ...
    console.log('Assets fetched successfully:', data.length)
    return data
  } catch (error) {
...
export default async function AssetsPage() {
  try {
    ...
    console.log('Session in AssetsPage:', {
      userId: session?.user?.id,
      email: session?.user?.email,
      role: session?.user?.role,
      expires: session?.expires
